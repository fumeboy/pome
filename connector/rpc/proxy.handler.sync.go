...
func (s *handler)sync(srv interface{}, serverStream grpc.ServerStream, serviceName, mthName string) (err error){
	outgoingCtx, backendConnAddr, err := s.sync_director(serverStream.Context(), serviceName, mthName)
	if err != nil {
		return err
	}
	...
	clientStream, err := grpc.NewClientStream(clientCtx, clientServerStreamDescForProxying, conn, utils.FullName(serviceName,mthName))
	if err != nil {
		return err
	}
