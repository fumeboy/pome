...
type (
	SyncDirectorT  = func(ctx context.Context, serviceName, mthName string) (context.Context, *grpc.ClientConn, error)
	AsyncDirectorT = func(serviceName, MethodName string,bytes []byte)
	handler        struct {
...
func ProxyHandler(director SyncDirectorT, director2 AsyncDirectorT) grpc.StreamHandler {
	streamer := &handler{director, director2}
